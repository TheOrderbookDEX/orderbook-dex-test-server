...
export function startOrderbookSimulation(address: string, verbose = false) {
    void (async function() {
        ...
        const tradedToken = IERC20WithFaucet.at(await orderbook.tradedToken());
        const baseToken = IERC20WithFaucet.at(await orderbook.baseToken());
        ...
        try {
            await signer.sendTransaction(await tradedToken.populateTransaction.faucet());
        } catch {
            // ignore
        }

        try {
            await signer.sendTransaction(await baseToken.populateTransaction.faucet());
        } catch {
            // ignore
        }
