...
export async function startServer({ dbPath, port = 8545, verbose }: ServerOptions): Promise<Server> {
    ...
    const logicRegistry    = await getContractAddress(blockNumber++, 0);
    const operatorFactory  = await getContractAddress(blockNumber++, 0);
    const operatorLogic    = await getContractAddress(blockNumber++, 0);
    ...
    return { addressBook, logicRegistry, operatorFactory, operatorLogic, orderbookFactory, tokens, orderbooks };
}
